...
gitlab-runner register \
	...
	--pre-build-script '/usr/bin/bash-static -c "/usr/bin/neofetch --backend off ; /root/print-free-space.sh"' \
...
gitlab-runner register \
	...
	--pre-build-script '/host/bash-static -c "/host/neofetch --backend off ; /host/fix-git-dubious-ownership.sh ; /host/print-free-space.sh"' \
	--env "$FF_ENABLE_JOB_CLEANUP" \
	--env "$FF_USE_FASTZIP" \
	--env "$FF_SCRIPT_SECTIONS"
